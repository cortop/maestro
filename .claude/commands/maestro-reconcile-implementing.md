...
wherever it changes what you do):
```bash
KEY="$1"
...
3. **Adversarial QA loop — an agent that did not write the code independently re-checks each
   ...
   *same batch* as the Spec-axis QA agent above, so they run in parallel — briefed with CLAUDE.md's
   ...
   - **Any AC verdict FAIL** → you (the implementer) fix the code per the QA evidence, append a
     turn breadcrumb (`maestro impl-turn "$KEY" --role implementer` — it numbers the turn and
     mints the step-id itself, so never hand-roll this with `maestro append`), re-run tests,
     then spawn QA again against the refreshed diff. Bound the rounds at `max_impl_turns`
     combined turns; this is enforced, not convention — crossing the ceiling parks the ticket
     via `fail` from inside `impl-turn`. If still failing when exhausted, do not open a PR —
