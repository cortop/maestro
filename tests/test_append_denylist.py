...
independent QA gate, tier-2 approval, backoff/dead-letter routing) was one raw
append away from irrelevant. `cmd_append` now refuses the seven ops-owned types with
an error naming the verb to use instead -- a denylist, not an allowlist, so the
legitimate raw appends (`events.SIDE_EFFECTING`, plus ad-hoc types like Note and
ResearchProposed) keep working unchanged.
"""
import pytest
