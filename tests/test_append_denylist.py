...
independent QA gate, tier-2 approval, backoff/dead-letter routing, RB-12's
captured-test-run gate) was one raw append away from irrelevant. `cmd_append` now
refuses the ops-owned types with an error naming the verb to use instead -- a
denylist, not an allowlist, so the legitimate raw appends (`events.SIDE_EFFECTING`,
...
# ---------------------------------------------------------------------------
# The ops-owned types refuse, name the owning verb, and append nothing.
# ---------------------------------------------------------------------------
...
def test_denylist_covers_exactly_the_seven_documented_types():
...
# ---------------------------------------------------------------------------
# End-to-end regression: a raw Approved can no longer smuggle a tier-2 ticket
# past the human-approval gate -- neither the snapshot nor a real dispatcher
# sweep is fooled.
# ---------------------------------------------------------------------------

def test_raw_approved_cannot_bypass_tier2_gate(home, cfg):
    seed_ticket(home, "T-1", "do the thing", phase="implementing", tier=2)
    ops.observe_spec(cfg, "T-1")  # sync spec_hash so dispatch sees "active", not "spec-changed"

    rc = cli.main(["--home", str(home), "append", "T-1", "--type", "Approved",
                   "--payload", "{}"])
